...
<?php
  ...
  function handleImageFIle_T1S2($img_start_T1S2, $img_end_T1S2, $chart_type_T1S2){
        $handle_task = fopen("img_csv/$chart_type_T1S2.csv",'r') or die("can't open file");
      ...
      for($i =0, $k = $img_start_T1S2; $k <= $img_end_T1S2; $k ++, $i++){
        if ($k  % 4 != 0) 
        {
          if(isset($images[$k]))
          {
    ...
    <div class="d-flex justify-content-center task-description">
      ...
      <div class="d-flex justify-content-center">
            <div class = "btn-group-justified btn-group-lg btn-group-vertical">
              <button type="button" id="btn_saturday_T1S2" class="btn btn-info">Sat</button>
              <button type="button" id="btn_sunday_T1S2" class="btn btn-info" style="margin-top: 15px;">Sun</button>
            </div>
    </div>
...
var trial_image_count_T1S2 = <?php echo $total_image_T1S2; ?>;
var image_index_T1S2 = 1;
...
console.log("Initital time counter", time_counter_0_T1S2);
show_images_T1S2(image_index_T1S2);
$('.trial_images_T1S2').unbind('click touch');

function set_current_time_T1S2(time_counter){
  last_time_count = time_counter;
}
function canvas_image_touch_deactive_T1S2(){
  $('.trial_images_T1S2').unbind('click touch');  
}
function button_enable_T1S2(){
    $('#btn_saturday_T1S2').prop('disabled', false);
    $('#btn_sunday_T1S2').prop('disabled', false);
}
function button_disable_T1S2(){
    $('#btn_saturday_T1S2').prop('disabled', true);
    $('#btn_sunday_T1S2').prop('disabled', true);     
}
...
function show_images_T1S2(n){
      ...
      var i;
      var slides_T1S2 = document.getElementsByClassName("trial_images_T1S2");
      if (n > slides_T1S2.length) {image_index_T1S2 = 1}
      if (n < 1) {image_index_T1S2 = slides_T1S2.length}
      for (i = 0; i < slides_T1S2.length; i++) {
          slides_T1S2[i].style.display = "none";
      }
      slides_T1S2[image_index_T1S2-1].style.display = "block";
      $('#current_trial_image_name_T1S2').val();
      // console.log(slides_T1S2[image_index_T1S2-1].id);
      $('#current_trial_image_name_T1S2').val(slides_T1S2[image_index_T1S2-1].id);
...
$("#btn_sunday_T1S2").click(function(){
    var current_trial_image_name_T1S2 = $('#current_trial_image_name_T1S2').val();
    ...
    elements_T1S2.push("t1");
    var start_pos = current_trial_image_name_T1S2.indexOf('_') + 1;
    var end_pos = current_trial_image_name_T1S2.indexOf('_',start_pos);
    var image_number = current_trial_image_name_T1S2.substring(start_pos,end_pos)
    // console.log("Trial No", image_number)
    sequence_T1S2.push(image_number);
    trial_T1S2.push(image_index_T1S2);

    var sliced_current_trial_image_name = current_trial_image_name_T1S2.slice(current_trial_image_name_T1S2.lastIndexOf('t1') + 2);
    var feedback_match = sliced_current_trial_image_name.substr(0, 1);
    if(feedback_match === 'r'){
      
      // var element = document.querySelector('[id^="img_T1S2_"]').id;
       $("#img_T1S2_"+current_trial_image_name_T1S2).css({"border-color": "green", 
         "border-width":"4px", 
         "border-style":"solid",
        //  "opacity":"0.5",
      ...
      $("#feedback_correct_T1S2_"+current_trial_image_name_T1S2).show();
      feedback_T1S2.push("correct");
      feedback_time_T1S2.push(time_counter_left - last_time_count);
    }else{

      $("#img_T1S2_"+current_trial_image_name_T1S2).css({"border-color": "red", 
         "border-width":"4px", 
         "border-style":"solid",
        //  "opacity":"0.5",
         "filter":"alpha(opacity=90)",
         });
      $("#feedback_error_T1S2_"+current_trial_image_name_T1S2).show();
      feedback_T1S2.push("error");
      feedback_time_T1S2.push(time_counter_left - last_time_count);
    }
...
$("#btn_saturday_T1S2").click(function(){
    var current_trial_image_name_T1S2 = $('#current_trial_image_name_T1S2').val();
    time_counter_left = performance.now();
    // console.log("Left button clicked" + (time_counter_left - last_time_count) + " milliseconds.")
    if( image_index_T1S2 === trial_image_count_T1S2){
...
$('body').on('next', function(e, type){
  ...
  var p_id_T1S2 = $('#participant_id').val();
  if (type === '<?php echo $id;?>'){
    $.ajax({
        type        : 'POST',  
        url         : 'ajax/test.php',  
        data        : { participant_id  : JSON.stringify(p_id_T1S2), 
                        chart           : JSON.stringify(chart_T1S2), 
                        elements        : JSON.stringify(elements_T1S2), 
                        trial           : JSON.stringify(trial_T1S2), 
                        sequence        : JSON.stringify(sequence_T1S2),
                        file            : JSON.stringify(file_T1S2), 
                        time            : JSON.stringify(feedback_time_T1S2), 
                        feedback        : JSON.stringify(feedback_T1S2) 
                      }, 
        dataType    : 'json',  
