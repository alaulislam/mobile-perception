...
<?php
  ...
  function handleImageFIle_T2S3($img_start_T2S3, $img_end_T2S3, $chart_type_T2S3){
        $handle_task = fopen("img_csv/$chart_type_T2S3.csv",'r') or die("can't open file");
      ...
      for($i =0, $k = $img_start_T2S3; $k <= $img_end_T2S3; $k ++, $i++){
        if ($k  % 4 != 0) 
        {
          if(isset($images[$k]))
          {
...
var trial_image_count_T2S3 = <?php echo $total_image_T2S3; ?>;
var image_index_T2S3 = 1;
...
console.log("Initital time counter", time_counter_0_T2S3);
show_images_T2S3(image_index_T2S3);
$('.trial_images_T2S3').unbind('click touch');

function set_current_time_T2S3(time_counter){
  last_time_count = time_counter;
}
function canvas_image_touch_deactive_T2S3(){
  $('.trial_images_T2S3').unbind('click touch');  
}
...
function show_images_T2S3(n){
      ...
      var i;
      var slides_T2S3 = document.getElementsByClassName("trial_images_T2S3");
      if (n > slides_T2S3.length) {image_index_T2S3 = 1}
      if (n < 1) {image_index_T2S3 = slides_T2S3.length}
      for (i = 0; i < slides_T2S3.length; i++) {
          slides_T2S3[i].style.display = "none";
      }
      slides_T2S3[image_index_T2S3-1].style.display = "block";
      $('#current_trial_image_name_T2S3').val();
      // console.log(slides_T2S3[image_index_T2S3-1].id);
      $('#current_trial_image_name_T2S3').val(slides_T2S3[image_index_T2S3-1].id);
...
$("#btn_sunday_T2S3").click(function(){
    ...
    var stimuli = current_trial_image_name_T2S3.substring(0,2);
    chart_T2S3.push(stimuli);
    elements_T2S3.push("t1");
    var start_pos = current_trial_image_name_T2S3.indexOf('_') + 1;
    var end_pos = current_trial_image_name_T2S3.indexOf('_',start_pos);
    var image_number = current_trial_image_name_T2S3.substring(start_pos,end_pos)
    ...
    if(feedback_match === 'r'){
      ...
      // var element = document.querySelector('[id^="img_T2S3_"]').id;
       $("#img_T2S3_"+current_trial_image_name_T2S3).css({"border-color": "green", 
         "border-width":"4px", 
         "border-style":"solid",
        //  "opacity":"0.5",
      ...
      $("#feedback_correct_T2S3_"+current_trial_image_name_T2S3).show();
      feedback_T2S3.push("correct");
      feedback_time_T2S3.push(time_counter_left - last_time_count);
    }else{

      $("#img_T2S3_"+current_trial_image_name_T2S3).css({"border-color": "red", 
         "border-width":"4px", 
         "border-style":"solid",
        //  "opacity":"0.5",
         "filter":"alpha(opacity=90)",
         });
      $("#feedback_error_T2S3_"+current_trial_image_name_T2S3).show();
      feedback_T2S3.push("error");
      feedback_time_T2S3.push(time_counter_left - last_time_count);
    }
...
$('body').on('next', function(e, type){
  ...
  var p_id_T2S3 = $('#participant_id').val();
  if (type === '<?php echo $id;?>'){
    $.ajax({
        type        : 'POST',  
        url         : 'ajax/test.php',  
        data        : { participant_id  : JSON.stringify(p_id_T2S3), 
                        chart           : JSON.stringify(chart_T2S3), 
                        elements        : JSON.stringify(elements_T2S3), 
                        trial           : JSON.stringify(trial_T2S3), 
                        sequence        : JSON.stringify(sequence_T2S3),
                        file            : JSON.stringify(file_T2S3), 
                        time            : JSON.stringify(feedback_time_T2S3), 
                        feedback        : JSON.stringify(feedback_T2S3) 
                      }, 
        dataType    : 'json',  
