<style>
   ...
   #gif_trial_images_T_T3S2D2 .btn:hover {
         ...
         <div class="d-flex justify-content-center" style="margin-top: 10px;" id="div_training_images_T_T3S2D2">
            ...
            <div class="slideshow-container">
               <div class="slideshow-inner justify-content-center ">
                  <div id="gif_trial_images_T_T3S2D2">
                     <img  src="img/exp_start2.jpg"/>
                     <button class="btn glow-on-hover" id="btn_trial_images_T_T3S2D2">Start!</button>
                  </div>
                  <div id="div_trial_images_T_T3S2D2" style="display:none">
                     <?php
                     ...
                     <?php } ?>
                     <div class="trial_images_T_T3S2D2" id="T3S2D2-<?php echo $i_T3S2D2; ?>-<?php echo $trial_image_shuffle_T_T3S2D2[$i_T3S2D2] ?>" >
                        <img id="img-T3S2D2-<?php echo $i_T3S2D2 ?>-<?php echo $trial_image_shuffle_T_T3S2D2[$i_T3S2D2] ?>" src="img/<?php echo $chart_type_T_T3S2D2; ?>/<?php echo $trial_image_shuffle_T_T3S2D2[$i_T3S2D2] ?>.png"/>
                     </div>
                     <?php } ?>
                  </div>
               </div>
...
<script type="text/javascript">
   ...
   var attention_check_flag_T_T3S2D2 = false;
   var attention_check_counter_T_T3S2D2 = 0;
   ...
   function show_images_T_T3S2D2(n){
         ...
         for (i = 0; i < slides_T_T3S2D2.length; i++) {
             slides_T_T3S2D2[i].style.display = "none";
         }
         slides_T_T3S2D2[image_index_T_T3S2D2-1].style.display = "block";
         $('#current_trial_image_name_T_T3S2D2').val();
         // console.log(slides_T_T3S2D2[image_index_T_T3S2D2-1].id);
         $('#current_trial_image_name_T_T3S2D2').val(slides_T_T3S2D2[image_index_T_T3S2D2-1].id);
   ...
   $("#btn_NO_T_T3S2D2").click(function(){
          ...
          var attention_check_img_T_T3S2D2 = current_trial_image_name_T_T3S2D2.startsWith("T3S2D2-attn-");
           if(attention_check_img_T_T3S2D2){
            attention_check_flag_T_T3S2D2 = true;
           }
         ...
         //   chart_T_T3S2D2.push(stimuli);
           ...
           var image_number = current_trial_image_name_T_T3S2D2.substring(start_pos,end_pos)
           sequence_T_T3S2D2.push(image_number);
           trial_T_T3S2D2.push(image_index_T_T3S2D2);
           ...
           if(feedback_match === 'r'){
            ...
            //  alert('wrong');
             ...
             feedback_time_T_T3S2D2.push(time_counter_NO_bn_T3S2D2 - last_time_count_T_T3S2D2);
             if(attention_check_flag_T_T3S2D2){
               attention_check_counter_T_T3S2D2+=1;
             }
           }
           attention_check_flag_T_T3S2D2 = false;
   ...
   $('body').on('next', function(e, type){
     ...
     if (type === '<?php echo $id;?>'){
      var global_attn_check_fail_count_T_T3S2D2       = parseInt($('#attention_check_fail_count').val());
      var curr_page_attn_check_fail_count_T_T3S2D2    = parseInt(attention_check_counter_T_T3S2D2);
      $('#attention_check_fail_count').val(global_attn_check_fail_count_T_T3S2D2 + curr_page_attn_check_fail_count_T_T3S2D2);
      $.ajax({
           type        : 'POST',  
           url         : 'ajax/experiment_data_save.php',  
           data        : { 
                                  page_name                   : JSON.stringify(page_name_T_T3S2D2), 
                                  participant_id              : JSON.stringify(participant_id_T_T3S2D2), 
                                  system_generated_id         : JSON.stringify(system_generated_id_T_T3S2D2), 
                                  experiment_sequence         : JSON.stringify(experiment_sequence_T_T3S2D2),
                                  experiment_order            : JSON.stringify(experiment_order_T_T3S2D2),
                                  is_main_trial               : JSON.stringify(is_main_trial_T_T3S2D2),
                                  task                        : JSON.stringify(task_T_T3S2D2),
                                  visualization_size          : JSON.stringify(chart_T_T3S2D2),
                                  trial_order                 : JSON.stringify(trial_T_T3S2D2),
                                  image_file_order            : JSON.stringify(sequence_T_T3S2D2),
                                  answering_time              : JSON.stringify(feedback_time_T_T3S2D2),
                                  answer                      : JSON.stringify(feedback_T_T3S2D2),
                                  image_file_name             : JSON.stringify(image_file_name_T_T3S2D2)
                         }, 
           dataType    : 'json',  
