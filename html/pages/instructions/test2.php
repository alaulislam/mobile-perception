...
<script type="text/javascript">
  ...
  //  $('.trial_images_T1S1D1').unbind('click touch');
   ...
   $("#btn_sunday_T1S1D1").click(function(){
       ...
       var attention_check = current_trial_image_name_T1S1D1.startsWith("T1S1D1-attn-");
       if(!attention_check){
   ...
   $('body').on('next', function(e, type){
    ...
    //  if (type === '<?php echo $id;?>'){
       if(attention_check_count_T1S1D1 < 2){
         excluded = true;
        var test_string = "Sending a string";
         $.ajax({
           type        : 'POST',  
           url         : 'ajax/attention_check_failed.php', 
           data:{'test_string':test_string}, 
          //  data        : { 
          //                   test_string  : JSON.stringify(test_string),
          //                }, 
           dataType    : 'json',  
