...
<?php
   ...
   function handleImageimage_P_T1S1D1($img_start_P_T1S1D1, $img_end_P_T1S1D1, $chart_type_P_T1S1D1){
       ...
       for($i =0, $k = $img_start_P_T1S1D1; $k <= $img_end_P_T1S1D1; $k += 4, $i++){
         if(isset($images[$k]))
         {
         ...
         <div class="d-flex justify-content-center" id="div_trainging_images_P_T1S1D1">
            ...
            <div class="slideshow-container">
               <div class="slideshow-inner justify-content-center">
                  <?php
                     for( $i_P_T1S1D1 = 0; $i_P_T1S1D1 < $total_image_P_T1S1D1; $i_P_T1S1D1++)
                     {  ?>                                     
                  <div class="trial_images_P_T1S1D1" id="P-T1S1D1-<?php echo $i_P_T1S1D1; ?>-<?php echo $trial_shuffle_P_T1S1D1[$i_P_T1S1D1] ?>">
                     <img id="img-P-T1S1D1-<?php echo $i_P_T1S1D1 ?>-<?php echo $trial_shuffle_P_T1S1D1[$i_P_T1S1D1] ?>" src="img/<?php echo $chart_type_P_T1S1D1 ?>/<?php echo $trial_shuffle_P_T1S1D1[$i_P_T1S1D1] ?>.png"/>
                  </div>
                  <?php } ?>
               </div>
            </div>
         </div>
         <div class="d-flex justify-content-center" id="div_trainging_ans_buttons_P_T1S1D1">
            <div class = "btn-group-justified btn-group-lg btn-group-vertical">
               <button type="button" id="btn_saturday_P_T1S1D1" class="btn btn-info">Sat</button>
               <button type="button" id="btn_sunday_P_T1S1D1" class="btn btn-info" style="margin-top: 15px;">Sun</button>
            </div>
         </div>
            ...
            <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
               <div class="modal-content h-auto rounded-right">
                  ...
                  <div class="modal-footer">
                     <p class="d-none" id="btn_next_correct_modal_message_P_T1S1D1">Training is over, wait a moment please!</p>
                     <button type="button" class="btn btn-dark border rounded-pill btn-wider w-100" id="btn_next_correct_modal_P_T1S1D1">NEXT</button>
                  </div>
               </div>
            </div>
         </div>
         <div class="modal left xs fade" id="modal_error_ans_P_T1S1D1" tabindex="-1" role="dialog" aria-labelledby="top_modal" data-keyboard="false" data-backdrop="static">
            <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
               <div class="modal-content h-auto rounded-right">
                  <div class="mt-1"></div>
                  <div class="modal-body text-center">
                  ...
                  <div style="margin-top:-15px"></div>
                  <div class="modal-footer">
                     <p class="d-none" id="btn_next_error_modal_message_P_T1S1D1">Training is over, wait a moment please!</p>
                     <button type="button" class="btn btn-dark border rounded-pill btn-wider w-100" id="btn_next_error_modal_P_T1S1D1">NEXT</button>
                  </div>
               </div>
            </div>
         </div>
...
<script type="text/javascript">
   ...
   $(document).ready(function() { 
     ...
     $("#btn_<?php echo $id;?>").hide();
      ...
      //  console.log("Initital time counter", time_counter_init_P_T1S1D1);
       ...
       function button_enable_P_T1S1D1(){
       ...
       function show_images_P_T1S1D1(n){
             ...
             var i_P_T1S1D1;
             var slides_P_T1S1D1 = document.getElementsByClassName("trial_images_P_T1S1D1");
             if (n > slides_P_T1S1D1.length) {image_index_P_T1S1D1 = 1}
             if (n < 1) {image_index_P_T1S1D1 = slides_P_T1S1D1.length}
             for (i_P_T1S1D1 = 0; i_P_T1S1D1 < slides_P_T1S1D1.length; i_P_T1S1D1++) {
                 slides_P_T1S1D1[i_P_T1S1D1].style.display = "none";
             }
             slides_P_T1S1D1[image_index_P_T1S1D1-1].style.display = "block";
             $('#current_trial_image_name_P_T1S1D1').val();
             // console.log(slides_P_T1S1D1[image_index_P_T1S1D1-1].id);
             $('#current_trial_image_name_P_T1S1D1').val(slides_P_T1S1D1[image_index_P_T1S1D1-1].id);
       }
       function next_images_P_T1S1D1(n){
           button_enable_P_T1S1D1();
           if (n < 0){
           show_images_P_T1S1D1(image_index_P_T1S1D1 -= 1);
           } else {
           show_images_P_T1S1D1(image_index_P_T1S1D1 += 1); 
           }
       }
       var progress_bar_interval_P_T1S1D1;
       function clear_modal_progress_bar_interval_P_T1S1D1(){
         clearInterval(progress_bar_interval_P_T1S1D1);
       }
       function fill_progress_bar_P_T1S1D1(answer){
         var progressBar =  $("#progress_bar_"+answer+"_P_T1S1D1"),
             width = 0;
             progressBar.width(width);
             progress_bar_interval_P_T1S1D1 = setInterval(function() {
                 width += 2;
                 progressBar.css('width', width + '%');
                 if (width >= 100) {
                     clearInterval(progress_bar_interval_P_T1S1D1);
                     if(answer === "correct"){
                       $('#modal_correct_ans_P_T1S1D1').fadeOut(100,function(){
                           $('#modal_correct_ans_P_T1S1D1').modal('hide');
                       });
                     }
                     if(answer === "error"){
                       $('#modal_error_ans_P_T1S1D1').fadeOut(100,function(){
                           $('#modal_error_ans_P_T1S1D1').modal('hide');
                       });
                     }
       ...
       $("#btn_sunday_P_T1S1D1").click(function(){
         ...
         //   var stimuli_P_T1S1D1 = current_trial_image_name_P_T1S1D1.substring(0,2);
           ...
           task_P_T1S1D1.push("t1");
           var start_pos_P_T1S1D1 = current_trial_image_name_P_T1S1D1.indexOf('_') + 1;
           var end_pos_P_T1S1D1 = current_trial_image_name_P_T1S1D1.indexOf('_',start_pos_P_T1S1D1);
           var image_number_P_T1S1D1 = current_trial_image_name_P_T1S1D1.substring(start_pos_P_T1S1D1,end_pos_P_T1S1D1)
           // console.log("Trial No", image_number_P_T1S1D1)
           sequence_P_T1S1D1.push(image_number_P_T1S1D1);
           trial_P_T1S1D1.push(image_index_P_T1S1D1);
   
           var sliced_current_trial_image_name_P_T1S1D1 = current_trial_image_name_P_T1S1D1.slice(current_trial_image_name_P_T1S1D1.lastIndexOf('t1') + 2);
           var feedback_match_P_T1S1D1 = sliced_current_trial_image_name_P_T1S1D1.substr(0, 1);
           if(feedback_match_P_T1S1D1 === 'r'){
             $('#modal_correct_ans_P_T1S1D1').modal('show');
             clear_modal_progress_bar_interval_P_T1S1D1();
             fill_progress_bar_P_T1S1D1("correct");
            $("#btn_next_correct_modal_P_T1S1D1").off('click').on('click', function(){
              clear_modal_progress_bar_interval_P_T1S1D1();
              $("#progress_bar_correct_P_T1S1D1").css('width', 0 + '%');
               $('#modal_correct_ans_P_T1S1D1').modal('toggle');
              //  $('#modal_correct_ans_P_T1S1D1').fadeOut(100,function(){ $('#modal_correct_ans_P_T1S1D1').modal('hide'); });
               next_images_P_T1S1D1(1);
             });
             feedback_P_T1S1D1.push("correct");
             feedback_time_P_T1S1D1.push(time_counter_left_P_T1S1D1 - last_time_count_P_T1S1D1);
           }else{
              $('#modal_error_ans_P_T1S1D1').modal('show');
              clear_modal_progress_bar_interval_P_T1S1D1();
              fill_progress_bar_P_T1S1D1("error");
              $("#btn_next_error_modal_P_T1S1D1").off('click').on('click', function(){
                clear_modal_progress_bar_interval_P_T1S1D1();
                $("#progress_bar_error_P_T1S1D1").css('width', 0 + '%');
                    $('#modal_error_ans_P_T1S1D1').modal('toggle');
       ...
       $("#btn_saturday_P_T1S1D1").click(function(){
         ...
         //   var stimuli_P_T1S1D1 = current_trial_image_name_P_T1S1D1.substring(8,2);
           ...
           var end_pos_P_T1S1D1 = current_trial_image_name_P_T1S1D1.indexOf('_',start_pos_P_T1S1D1);
           var image_number_P_T1S1D1 = current_trial_image_name_P_T1S1D1.substring(start_pos_P_T1S1D1,end_pos_P_T1S1D1)
           // console.log("Image Seq", image_number_P_T1S1D1)
           sequence_P_T1S1D1.push(image_number_P_T1S1D1);
           trial_P_T1S1D1.push(image_index_P_T1S1D1);
            ...
            $("#btn_next_correct_modal_P_T1S1D1").off('click').on('click', function(){
             ...
             $('#modal_error_ans_P_T1S1D1').modal('show');
              ...
              $("#btn_next_error_modal_P_T1S1D1").off('click').on('click', function(){
                clear_modal_progress_bar_interval_P_T1S1D1();
                $("#progress_bar_error_P_T1S1D1").css('width', 0 + '%');
              // $("#btn_next_error_modal_P_T1S1D1").click(function(){
               $('#modal_error_ans_P_T1S1D1').fadeOut(100,function(){$('#modal_error_ans_P_T1S1D1').modal('hide'); });
               next_images_P_T1S1D1(1);
              });
             feedback_P_T1S1D1.push("error");
   ...
   $('body').on('next', function(e, type){
     ...
     event.preventDefault();
      var page_name_P_T1S1D1                   = '<?php echo $id;?>';
     ...
     if (type === '<?php echo $id;?>'){
       $.ajax({
           ...
           data        : { 
                                  page_name                   : JSON.stringify(page_name_P_T1S1D1), 
                                  participant_id              : JSON.stringify(participant_id_P_T1S1D1), 
                                  system_generated_id         : JSON.stringify(system_generated_id_P_T1S1D1), 
                                  experiment_sequence         : JSON.stringify(experiment_sequence_P_T1S1D1),
                                  experiment_order            : JSON.stringify(experiment_order_P_T1S1D1),
                                  is_main_trial               : JSON.stringify(is_main_trial_P_T1S1D1),
                                  task                        : JSON.stringify(task_P_T1S1D1),
                                  visualization_size          : JSON.stringify(chart_P_T1S1D1),
                                  trial_order                 : JSON.stringify(trial_P_T1S1D1),
                                  image_file_order            : JSON.stringify(sequence_P_T1S1D1),
                                  answering_time              : JSON.stringify(feedback_time_P_T1S1D1),
                                  answer                      : JSON.stringify(feedback_P_T1S1D1),
                                  image_file_name             : JSON.stringify(image_file_name_P_T1S1D1)
                         }, 
           dataType    : 'json',  
