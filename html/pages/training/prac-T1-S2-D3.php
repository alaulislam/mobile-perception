...
<?php
   ...
   $experiment_order_P_T1S2D3 ="T1-S2-D3";
   ...
   $trial_shuffle_P_T1S2D3 = handleImageimage_P_T1S2D3($img_start_P_T1S2D3, $img_end_P_T1S2D3, $chart_type_P_T1S2D3);
     
   function handleImageimage_P_T1S2D3($img_start_P_T1S2D3, $img_end_P_T1S2D3, $chart_type_P_T1S2D3){
...
<div class="row">
   <div class="col">
      <div class="container">
         <div class=mt-3></div>
         <h5 id="header_question_P_T1S2D3">On which day did you sleep longer, Saturday or Sunday?</h5>
         <div class="d-flex justify-content-center" id="div_trainging_images_P_T1S2D3">
         ...
         <div class="d-flex justify-content-center" id="div_trainging_ans_buttons_P_T1S2D3">
         ...
         <div class="row mt-4 d-none" id="end_message_P_T1S2D3">
         ...
         <div class=mt-4></div>
         <div class="modal left xs fade" id="modal_correct_ans_P_T1S2D3" tabindex="-1" role="dialog" aria-labelledby="top_modal" data-keyboard="false" data-backdrop="static">
            <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
               <div class="modal-content h-auto rounded-right">
                  <div class="mt-1"></div>
                  <div class="modal-header d-none" id="modal_correct_dismiss_P_T1S2D3">
                     <!-- <h5 class="modal-title"><i class="fa fa-cog"></i> Preferences</h5> -->
                     <button type="button" class="close" id="modal_correct_dismiss_btn_P_T1S2D3"  data-dismiss="modal" aria-label="Close">
                  ...
                  <div class="modal-body text-center">
                     ...
                     <div class="progress">
                        <div class="progress-bar bg-info" id="progress_bar_correct_P_T1S2D3" role="progressbar" style="width: 0%" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
                     </div>
                  </div>
                  <div style="margin-top:-15px"></div>
                  <div class="modal-footer">
                     <p class="d-none" id="btn_next_correct_modal_message_P_T1S2D3">Training is over, wait a moment please!</p>
                     <button type="button" class="btn btn-dark border rounded-pill btn-wider w-100" id="btn_next_correct_modal_P_T1S2D3">NEXT</button>
                  </div>
               </div>
            </div>
         </div>
         <div class="modal left xs fade" id="modal_error_ans_P_T1S2D3" tabindex="-1" role="dialog" aria-labelledby="top_modal" data-keyboard="false" data-backdrop="static">
            <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
               <div class="modal-content h-auto rounded-right">
                  <div class="mt-1"></div>
                  <div class="modal-body text-center">
                     ...
                     <div class="progress">
                        <div class="progress-bar bg-info" id="progress_bar_error_P_T1S2D3" role="progressbar" style="width: 0%" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
                     </div>
                  </div>
                  <div style="margin-top:-15px"></div>
                  <div class="modal-footer">
                     <p class="d-none" id="btn_next_error_modal_message_P_T1S2D3">Training is over, wait a moment please!</p>
                     <button type="button" class="btn btn-dark border rounded-pill btn-wider w-100" id="btn_next_error_modal_P_T1S2D3">NEXT</button>
                  </div>
               </div>
            </div>
         </div>
...
<script type="text/javascript">
   ...
   var viz_size_P_T1S2D3 = '<?php echo $chart_type_P_T1S2D3 ?>';
   var stimuli_P_T1S2D3 = viz_size_P_T1S2D3.toLowerCase();
   var image_index_P_T1S2D3 = 1;
   var image_file_name_P_T1S2D3 = [];
   var chart_P_T1S2D3 = [];
   var task_P_T1S2D3 = [];
   var trial_P_T1S2D3 = [];
   ...
   $(document).ready(function() { 
     ...
     $("#btn_<?php echo $id;?>").hide();
       var time_counter_init_P_T1S2D3 = performance.now();
       set_current_time_P_T1S2D3(time_counter_init_P_T1S2D3);
      //  console.log("Initital time counter", time_counter_init_P_T1S2D3);
       ...
       function show_images_P_T1S2D3(n){
             if( image_index_P_T1S2D3 === trial_image_count_P_T1S2D3){
                $("#btn_next_correct_modal_P_T1S2D3").hide();
                $("#btn_next_error_modal_P_T1S2D3").hide();
                $('#btn_next_correct_modal_message_P_T1S2D3').removeClass("d-none");
                $('#btn_next_error_modal_message_P_T1S2D3').removeClass("d-none");
             }
             if( image_index_P_T1S2D3 > trial_image_count_P_T1S2D3){
              $("#div_trainging_images_P_T1S2D3").children().hide();
              $("#div_trainging_ans_buttons_P_T1S2D3").children().hide();
              $('#end_message_P_T1S2D3').removeClass("d-none");
              $("#header_question_P_T1S2D3").hide();
       ...
       var progress_bar_interval_P_T1S2D3;
       function clear_modal_progress_bar_interval_P_T1S2D3(){
         clearInterval(progress_bar_interval_P_T1S2D3);
       }
       function fill_progress_bar_P_T1S2D3(answer){
         var progressBar =  $("#progress_bar_"+answer+"_P_T1S2D3"),
             width = 0;
             progressBar.width(width);
             progress_bar_interval_P_T1S2D3 = setInterval(function() {
                 width += 2;
                 progressBar.css('width', width + '%');
                 if (width >= 100) {
                     clearInterval(progress_bar_interval_P_T1S2D3);
                     if(answer === "correct"){
                       $('#modal_correct_ans_P_T1S2D3').fadeOut(100,function(){
                           $('#modal_correct_ans_P_T1S2D3').modal('hide');
                       });
                     }
                     if(answer === "error"){
                       $('#modal_error_ans_P_T1S2D3').fadeOut(100,function(){
                           $('#modal_error_ans_P_T1S2D3').modal('hide');
                       });
                     }
       ...
       $("#btn_sunday_P_T1S2D3").click(function(){
         ...
         //   var stimuli_P_T1S2D3 = current_trial_image_name_P_T1S2D3.substring(0,2);
           ...
           if(feedback_match_P_T1S2D3 === 'r'){
             $('#modal_correct_ans_P_T1S2D3').modal('show');
             clear_modal_progress_bar_interval_P_T1S2D3();
             fill_progress_bar_P_T1S2D3("correct");
            $("#btn_next_correct_modal_P_T1S2D3").off('click').on('click', function(){
              clear_modal_progress_bar_interval_P_T1S2D3();
              $("#progress_bar_correct_P_T1S2D3").css('width', 0 + '%');
               $('#modal_correct_ans_P_T1S2D3').modal('toggle');
              //  $('#modal_correct_ans_P_T1S2D3').fadeOut(100,function(){ $('#modal_correct_ans_P_T1S2D3').modal('hide'); });
               next_images_P_T1S2D3(1);
             });
             feedback_P_T1S2D3.push("correct");
             feedback_time_P_T1S2D3.push(time_counter_left_P_T1S2D3 - last_time_count_P_T1S2D3);
           }else{
              $('#modal_error_ans_P_T1S2D3').modal('show');
              clear_modal_progress_bar_interval_P_T1S2D3();
              fill_progress_bar_P_T1S2D3("error");
              $("#btn_next_error_modal_P_T1S2D3").off('click').on('click', function(){
                clear_modal_progress_bar_interval_P_T1S2D3();
                $("#progress_bar_error_P_T1S2D3").css('width', 0 + '%');
                    $('#modal_error_ans_P_T1S2D3').modal('toggle');
              //  $('#modal_error_ans_P_T1S2D3').fadeOut(100,function(){$('#modal_error_ans_P_T1S2D3').modal('hide'); });
               next_images_P_T1S2D3(1);
             });
       ...
       $("#btn_saturday_P_T1S2D3").click(function(){
           ...
           image_file_name_P_T1S2D3.push(current_trial_image_name_P_T1S2D3);
         //   var stimuli_P_T1S2D3 = current_trial_image_name_P_T1S2D3.substring(8,2);
           chart_P_T1S2D3.push(stimuli_P_T1S2D3);
           task_P_T1S2D3.push("t1");
           var start_pos_P_T1S2D3 = current_trial_image_name_P_T1S2D3.indexOf('_') + 1;
           var end_pos_P_T1S2D3 = current_trial_image_name_P_T1S2D3.indexOf('_',start_pos_P_T1S2D3);
            ...
            $("#btn_next_correct_modal_P_T1S2D3").off('click').on('click', function(){
             ...
             $('#modal_error_ans_P_T1S2D3').modal('show');
              ...
              $("#btn_next_error_modal_P_T1S2D3").off('click').on('click', function(){
                clear_modal_progress_bar_interval_P_T1S2D3();
                $("#progress_bar_error_P_T1S2D3").css('width', 0 + '%');
              // $("#btn_next_error_modal_P_T1S2D3").click(function(){
               $('#modal_error_ans_P_T1S2D3').fadeOut(100,function(){$('#modal_error_ans_P_T1S2D3').modal('hide'); });
               next_images_P_T1S2D3(1);
              });
             feedback_P_T1S2D3.push("error");
   ...
   $('body').on('next', function(e, type){
     ...
     event.preventDefault();
      var page_name_P_T1S2D3                   = '<?php echo $id;?>';
      var participant_id_P_T1S2D3              = $('#participant_id').val();
      var system_generated_id_P_T1S2D3         = $('#system_generated_id').val();
      var experiment_sequence_P_T1S2D3         = '<?php echo $between_subject_sequence;?>';
      var experiment_order_P_T1S2D3            = '<?php echo $experiment_order_P_T1S2D3;?>';
      var is_main_trial_P_T1S2D3               = 0;
     if (type === '<?php echo $id;?>'){
       $.ajax({
           type        : 'POST',  
           url         : 'ajax/experiment_data_save.php',  
           data        : { 
                                  page_name                   : JSON.stringify(page_name_P_T1S2D3), 
                                  participant_id              : JSON.stringify(participant_id_P_T1S2D3), 
                                  system_generated_id         : JSON.stringify(system_generated_id_P_T1S2D3), 
                                  experiment_sequence         : JSON.stringify(experiment_sequence_P_T1S2D3),
                                  experiment_order            : JSON.stringify(experiment_order_P_T1S2D3),
                                  is_main_trial               : JSON.stringify(is_main_trial_P_T1S2D3),
                                  task                        : JSON.stringify(task_P_T1S2D3),
                                  visualization_size          : JSON.stringify(chart_P_T1S2D3),
                                  trial_order                 : JSON.stringify(trial_P_T1S2D3),
                                  image_file_order            : JSON.stringify(sequence_P_T1S2D3),
                                  answering_time              : JSON.stringify(feedback_time_P_T1S2D3),
                                  answer                      : JSON.stringify(feedback_P_T1S2D3),
                                  image_file_name             : JSON.stringify(image_file_name_P_T1S2D3)
                         }, 
           dataType    : 'json',  
