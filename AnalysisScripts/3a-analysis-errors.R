...
for (task in tasks){
  # if(task=="T1" || task=="T2") next
  # setwd("~/Desktop/OnlineExperiment/AnalysisScripts/results/")
  filename <- paste("~/Desktop/OnlineExperiment/AnalysisScripts/results/aggregated-",task,".csv",sep="")
  errors_table <- read.csv(filename)
  print(errors_table)
  filename_analysis <- paste("~/Desktop/OnlineExperiment/AnalysisScripts/results/error-analysis-",task,sep="")
  ...
  elements <- mydata
  print(elements)
  # elements <- elements [ order(elements$participant_id, elements$sizes), ]
  elements <- elements [ order(elements$participant_id, elements$visualization_size), ]
  #
  myvars <- c("participant_id", "visualization_size", "mean_correct", "mean_time_taken")
  ...
  colnames(elements) <- gsub("mean_correct. ","", colnames(elements))
  ...
  colnames(datatoprint) <- c("vis_size", "mean_correct", "lowerBound_CI", "upperBound_CI ")
  ...
  barChart(datatoprint,analysisData$name ,nbTechs = 3, ymin = 0, ymax = 1, "", "Avg. Correct. Error Bars, Bootstrap 95% CIs")
  
  ggsave(paste("~/Desktop/OnlineExperiment/AnalysisScripts/plots/correct-means-",task,".pdf",sep=""),device = "pdf", width=5, height=2)
  ...
  colnames(datatoprint) <- c("visualization_size", "mean_correct", "upperBound_CI","lowerBound_CI", "corrected_CI", "upperBound_CI_corr", "lowerBound_CI_corr") #We use the name mean_time for the value of the mean even though it's not a time, it's just to parse the data for the plot
  
  
  tmp <- paste(filename_analysis, paste("diffs-",task,".csv"), collapse="_")
  write.csv(datatoprint,file=tmp)
  ...
  barChart_corr(datatoprint, analysisData$name, nbTechs = 3, ymin = -.1, ymax = .1, "", "")
  ggsave(paste("~/Desktop/OnlineExperiment/AnalysisScripts/plots/error-diffs-",task,".pdf"),device = "pdf", width=5, height=2)
