...
bootstrapCI <- function(statistic, datapoints) {
  ...
  if(length(unique(datapoints)) <= 1){ #same value for all responses
    print("Returning fake CI")
    return(c(pointEstimate,pointEstimate,pointEstimate))    
  }
...
bootstrapCI_corr <- function(statistic, datapoints, comparisons) {
  ...
  if(length(unique(datapoints)) < 2){ #sane value for all responses
    print("Returning fake CI")
    return(c(pointEstimate,pointEstimate,pointEstimate,level,pointEstimate,pointEstimate))    
  }
...
barChart_corr <- function(resultTable, techniques, nbTechs = -1, ymin, ymax, xAxisLabel = "I am the X axis", yAxisLabel = "I am the Y Label"){
